...
export const analyzeStatement = async (file: File): Promise<AnalysisResult> => {
  ...
  const responseSchema = {
    ...
    properties: {
      ...
      timeshareNo: { type: Type.STRING, description: "Timeshare Number of the guest, often found near guest name or room" },
      ...
      transactions: {
        ...
        items: {
          ...
          properties: {
            ...
            originalDescription: { 
              type: Type.STRING,
              description: "The full, raw description from the statement. MUST include all details: CHECK#, room numbers, employee IDs, or any other codes. DO NOT OMIT ANYTHING."
            },
            cleanName: { 
              type: Type.STRING, 
  ...
  const systemInstruction = `
    ...
    CRITICAL RULES:
    1. Header Information: Extract Hotel Name, Timeshare Number, Guest Name, Room Number, Folio/Confirmation number accurately from the top of the document. For dates like Check-In and Check-Out, convert formats like '19-01-26' to a standard 'DD/MM/YYYY' format (e.g., '19/01/2026').
    2. Transactions: Extract the Date, Description, and Amount for each line item in the main table. Ensure the 'originalDescription' includes ALL text from the description column, including check numbers (e.g., 'CHECK# 1234'), room numbers, and any other specific details. Do not omit any information. This is critical for the user's reconciliation.
    3. 'cleanName': From the original description, create a simplified commercial name. Remove dates, transaction codes, room numbers, and any other non-essential text. For example, 'REST EL PATIO 23423' should become 'El Patio'. Keep the 'originalDescription' as the source of truth for all details.
